...
export const signup = async (req, res) => {
  Joi.validate(req.body, signUnSchema)
    .then(async () => {
      ...
      if (!foundUser) {
        ...
        res.status(409).send({ status: 409, error: 'Already signed up user' });
    })
    .catch(e => {
      if (e.isJoi)
        res.status(400).send({ status: 400, error: e.details[0].message });
...
export const signin = async (req, res) => {
  ...
  if (foundUser) {
    ...
    bcrypt.compare(password, foundUser.password, (err, result) => {
      if (err)
        res.status(401).send({ status: 401, error: 'Incorrect credentials' });
      else if (!result)
        return res
          .status(401)
          .send({ status: 401, error: 'Incorrect credentials' });
      else {
        const token = jwt.sign({ email }, SECRET, { expiresIn: '3h' });
        res
          .status(200)
          .send({ status: 200, data: getPublicProfile(foundUser), token });
      }
    });
  } else
    return res.status(404).send({ status: 404, error: 'Email not registred' });
};
